...
// mamy areny i w tych arenach trzymamy bloki
static arenalist_t arenas = STAILQ_HEAD_INITIALIZER(arenas);
static arena_t *cur_arena = NULL;
static unsigned cur_offset;

// wypełniamy zarządzany obszar ujemnymi (wolnymi) polami
// każdy węzeł tworzy blok o rozmiarze MAX_LENGTH + 1
...
static void *alloc_block(int8_t *data, uint8_t len) {
  ...
  while (!END_OF_CHUNK(data)) {
    if (BLOCK_USED(data)) {
      /* TODO: occupied block */
      MOVE_NEXT(data); // szukamy spełniającego, idziemy dalej
      ...
      // splitting
      assert(BLOCK_HEADER(data) < 0);
      int8_t split = BLOCK_HEADER(data) + len;
      assert(split < 0);
      BLOCK_HEADER(data) = len;
      result = CURRENT_PTR(data);
      NEXT_BLOCK_HEADER(data) = -split; // zaczynamy nasz wolny blok w innym miejscu
      assert(BLOCK_HEADER(data) > 0);
...
static void strfree(char *str) {
  ...
  if (*ptr > 0){
    *ptr *= -1; // zmieniamy node'a po prostu na ujemnego, wolnego
  }
}
