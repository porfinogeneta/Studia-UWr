...
int main(int argc, char **argv) {
  ...
  pthread_t lst_thrd[nthreads];

  // uruchamiamy nthreads wątków
  for (int i = 0; i < nthreads; i++){
    Pthread_create(&lst_thrd[i], NULL, thread, NULL);
  }
  ...
  // czekamy na ich zakończenie w wątku głównym, staje się on wątkiem detached
  for (int i = 0; i < nthreads; i++){
    void *return_value;
    Pthread_join(lst_thrd[i], &return_value);
    printf("Thread %d finished. Return value: %ld\n", i, (long)(intptr_t)return_value);
  }
