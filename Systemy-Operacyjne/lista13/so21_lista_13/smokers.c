...
static void *smokerWithMatches(void *arg) {
  ...
  while (true) {
    ...
    // make_and_smoke('M');
    // Sem_post(&mutex);
    if ((sem_trywait(&paper) && sem_trywait(&tobacco)) == 0){
      make_and_smoke('M');
    }
...
static void *smokerWithTobacco(void *arg) {
  ...
  while (true) {
    ...
    // make_and_smoke('T');
    // Sem_post(&mutex);
    if ((sem_trywait(&paper) && sem_trywait(&matches)) == 0){
      make_and_smoke('T');
    }
  }
...
static void *smokerWithPaper(void *arg) {
  ...
  while (true) {
    ...
    // make_and_smoke('P');
    // Sem_post(&mutex);
    if ((sem_trywait(&tobacco) && sem_trywait(&matches)) == 0){
      make_and_smoke('P');
    }
...
int main(void) {
  ...
  // Sem_init(&mutex, 0, 1);
  // zrobić wątki, przekazujące składniki palaczom, mamy 3 wątki informujące palaczy
