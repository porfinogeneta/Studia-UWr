...
(define (wire-set! wire new-value)
  ...
  (if (not (eq? (wire-value wire) new-value))
      ...
      (begin
        ...
        (for-each (lambda (action) (action)) (wire-on-change-procedures wire))
        )
...
(define (wire-and in1 in2)
  (let ([out (make-wire (wire-sim in1))]) ;musimy być dalej w tej samej symulacji, co kable wejścia na kablu wyjścia
